...
목표:
1. 기존 미니게임 홈에 로컬 프로필/기록 기능, `미로찾기`, `사진 퍼즐` 게임이 있어야 한다.
2. 로컬 프로필/기록은 서버 없이 `localStorage`에 저장한다.
3. `미로찾기`는 랜덤 미로, 난이도, 힌트, 키보드/버튼 이동, 바깥 입구/출구, 오리지널 숫자 블록 캐릭터를 지원해야 한다.
4. `사진 퍼즐`은 사진 또는 동영상을 선택해 줄/칸 수대로 퍼즐을 만들고, PC 드래그 앤 드롭과 모바일 터치 선택 배치를 지원해야 한다.
5. 동영상을 선택하면 첫 프레임을 캔버스로 캡처해 퍼즐 이미지로 쓰고, 퍼즐 완성 후 원본 동영상을 표시 및 재생해야 한다.
6. 미로찾기와 사진/동영상 퍼즐 완료 시 현재 유저의 플레이 기록을 저장해야 한다.
7. `/docs`에는 프로젝트 맥락, 작업내역, 기능명세, 다음 작업, AI 작업 가이드, 구현 청사진, 재현 프롬프트, QA 체크리스트가 있어야 한다.
8. 작업 후 결과물을 `C:\Users\PC\vs-workspace\game\game`으로 복사해야 한다.

로컬 프로필/기록 구현 조건:
- 객체 이름: `gameRecords`
- 저장 키: `miniGamePlayRecords.v1`
- 데이터 구조: `{ users: [], activeUserId: '', records: [] }`
- 유저가 없으면 `게스트`를 자동 생성한다.
- 홈 화면에 현재 플레이어, 프로필 선택, 새 이름 입력, 추가 버튼, 기록 보기 버튼이 있어야 한다.
- 기록 화면 id: `records-view`
- 기록 화면에는 현재 플레이어의 기록만 표시한다.
- 기록 공통 필드:
  - id
  - userId
  - gameType
  ...
  - hintsUsed
  - stats
- 각 게임은 완료 시 `gameRecords.recordGameResult(record)`만 호출한다.
...
- 완료 시 미로 기록을 저장한다.
- 미로 stats에는 `steps`, `mazeSize`, `exitX`, `exitY`를 넣는다.
...
- 완료 시 사진/동영상 퍼즐 기록을 저장한다.
- 퍼즐 stats에는 `rows`, `cols`, `pieces`, `moves`, `mediaType`을 넣는다.
